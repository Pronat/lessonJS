...
// })
//
// const promiseSuper3 = Promise.race([promise1, promise2, promise3])  //получаем резолв или реджект с любым первым ответившим сервером
// promiseSuper3
//     .then((data)=>{
//     console.log(data)
// })
...
const someFunction = async () => {
    try {
        const dataFromGoogle = fetch('https://googfle.com/query=js')
        const dataFromYahoo = fetch('https://yahoo.com/query=js')
        const dataFromDuck = fetch('https://duckduckgo.com/query=js')
    }
    catch (err) {
        console.log('error from await', err)
    }
}
